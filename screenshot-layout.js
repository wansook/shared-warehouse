...
(async () => {
  ...
  // Click admin layout button
  const editButton = page.locator('button:has-text("Edit layout"), button:has-text("레이아웃 편집"), [class*="edit-layout"]');
  if (await editButton.count() > 0) {
    await editButton.first().click();
    await new Promise(r => setTimeout(r, 2000));
  }
  
  await page.screenshot({ path: 'C:\\git\\shared-warehouse\\screens\\layout_editor_showcase.png', fullPage: false });
  console.log('Saved!');
