# codex-output-language-ko modified files
...
const Dashboard = () => {
  ...
  const handleAddWarehouse = async (e) => {
    ...
    try {
      ...
      setMessage('李쎄퀬媛 異붽??섏뿀?듬땲??');
      setNewWarehouse({ name: '', location: '', capacity: 0 });
      setShowAddWarehouse(false);
      fetchWarehouses();
    } catch (error) {
      setMessage(error.response?.data?.message || '李쎄퀬 異붽????ㅽ뙣?덉뒿?덈떎.');
    }
  };
  ...
  const handleAuthenticate = async (e) => {
    ...
    try {
      ...
      setAuthResult(`?깃났: ${response.data.message}`);
      fetchAccessLogs(selectedWarehouse);
    } catch (error) {
      setAuthResult(`?ㅽ뙣: ${error.response?.data?.message || '?몄쬆???ㅽ뙣?덉뒿?덈떎.'}`);
    }
  };
  ...
  const handleUnlockDoor = async (warehouseId) => {
    try {
      ...
      setMessage('臾몄씠 ?대졇?듬땲??');
      fetchHardwareStatus();
    } catch (error) {
      setMessage(error.response?.data?.message || '臾??닿린???ㅽ뙣?덉뒿?덈떎.');
    }
  };
  ...
  const handleSyncEmails = async () => {
    ...
    try {
      ...
      setMessage(error.response?.data?.message || '?숆린?붿뿉 ?ㅽ뙣?덉뒿?덈떎.');
    } finally {
    ...
    <div className="dashboard">
      <header className="dashboard-header">
        ...
        <div className="header-right">
          <span className="user-name">{user?.username} {isAdmin && <span className="admin-badge">愿由ъ옄</span>}</span>
          <button className="profile-btn" onClick={() => navigate('/profile')}>?꾨줈??/button>
          <button className="logout-btn" onClick={handleLogout}>濡쒓렇?꾩썐</button>
        </div>
      </header>
      ...
      <div className="dashboard-content">
        <div className="warehouses-section">
          <div className="section-header">
            <h2>李쎄퀬</h2>
            {isAdmin && <button onClick={() => setShowAddWarehouse(true)}>+ 李쎄퀬</button>}
          </div>

          {showAddWarehouse && (
            <form onSubmit={handleAddWarehouse} className="add-form">
              <div className="form-group">
                <input type="text" placeholder="?대쫫" value={newWarehouse.name} onChange={(e) => setNewWarehouse({ ...newWarehouse, name: e.target.value })} required />
              </div>
              <div className="form-group">
                <input type="text" placeholder="?꾩튂" value={newWarehouse.location} onChange={(e) => setNewWarehouse({ ...newWarehouse, location: e.target.value })} />
              </div>
              <div className="form-group">
                <input type="number" placeholder="?섏슜?? value={newWarehouse.capacity} onChange={(e) => setNewWarehouse({ ...newWarehouse, capacity: parseInt(e.target.value, 10) || 0 })} />
              </div>
              <div className="form-actions">
                <button type="submit">???/button>
                <button type="button" onClick={() => setShowAddWarehouse(false)}>痍⑥냼</button>
              </div>
            </form>
          )}
            ...
            {warehouses.map((warehouse) => (
              <div key={warehouse.id} className={`warehouse-card ${Number(selectedWarehouse) === Number(warehouse.id) ? 'active' : ''}`} onClick={() => selectWarehouse(warehouse)}>
                <div className="warehouse-info">
                  ...
                  <p>?꾩튂: {warehouse.location || '-'}</p>
                  <p>?섏슜?? {warehouse.capacity}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {selectedWarehouse && (
            <div className="auth-section">
              <div className="section-header">
                <h2>異쒖엯 ?몄쬆</h2>
                <button onClick={() => setShowAuthPanel(!showAuthPanel)}>{showAuthPanel ? '?リ린' : '?몄쬆 ?뚯뒪??}</button>
              </div>
              ...
              {showAuthPanel && (
                <div className="auth-panel">
                  <form onSubmit={handleAuthenticate} className="auth-form">
                    ...
                    <div className="form-group">
                      <input type="text" placeholder="?몄쬆 媛? value={authData.value} onChange={(e) => setAuthData({ ...authData, value: e.target.value })} required />
                    </div>
                    <button type="submit">?몄쬆</button>
                  </form>
              ...
              <div className="logs-list">
                {accessLogs.map((log) => (
                  <div key={log.id} className={`log-item ${log.success ? 'success' : 'failed'}`}>
                    <span className="log-status">{log.success ? '?깃났' : '?ㅽ뙣'}</span>
                    <span>{log.username || '?????놁쓬'}</span>
        ...
        <div className="right-panel">
          <div className="tab-nav">
            <button className={activeTab === 'cabinets' ? 'active' : ''} onClick={() => setActiveTab('cabinets')}>罹먮퉬??/button>
            <button className={activeTab === 'contracts' ? 'active' : ''} onClick={() => setActiveTab('contracts')}>怨꾩빟</button>
            {isAdmin && <button className={activeTab === 'hardware' ? 'active' : ''} onClick={() => { setActiveTab('hardware'); fetchHardwareStatus(); }}>?섎뱶?⑥뼱</button>}
            {isAdmin && <button className={activeTab === 'naver' ? 'active' : ''} onClick={() => { setActiveTab('naver'); fetchNaverReservations(); }}>?ㅼ씠踰?/button>}
          </div>

          {stats && (
            <div className="stats-card">
              <h3>{currentWarehouse?.name} ?듦퀎</h3>
              <div className="stats-grid">
                <div className="stat-item"><span className="stat-label">?덈ぉ</span><span className="stat-value">{stats.total_items}</span></div>
                <div className="stat-item"><span className="stat-label">?섎웾</span><span className="stat-value">{stats.total_quantity}</span></div>
              </div>
            </div>
          )}
          ...
          {searchTerm && (
            <div className="search-results">
              <h3>寃??寃곌낵 ({searchResults.length})</h3>
          ...
          {activeTab === 'cabinets' && selectedWarehouse && (
            <div className="cabinets-section">
              <div className="section-header">
                <h2>罹먮퉬??諛곗튂</h2>
                <div>
                  {isAdmin && <button onClick={() => navigate('/layout-editor', { state: { warehouseId: selectedWarehouse } })}>諛곗튂 ?몄쭛</button>}
                  {isAdmin && <button onClick={() => setShowAddCabinet(true)}>+ 罹먮퉬??/button>}
                </div>
              </div>
              ...
              {showAddCabinet && (
                <form onSubmit={handleAddCabinet} className="add-form">
                  ...
                  <div className="form-group">
                    <input type="number" placeholder="由대젅??梨꾨꼸" value={newCabinet.relay_channel} onChange={(e) => setNewCabinet({ ...newCabinet, relay_channel: parseInt(e.target.value, 10) || 1 })} min="1" max="4" />
                  </div>
                  <div className="form-actions">
                    <button type="submit">???/button>
                    <button type="button" onClick={() => setShowAddCabinet(false)}>痍⑥냼</button>
                  </div>
                </form>
              )}
              ...
              <div className="layout-viewer">
                <div className="warehouse-floor">
                  <div className="floor-label">{currentWarehouse?.name || '李쎄퀬'}</div>
          ...
          {activeTab === 'contracts' && (
            <div className="contracts-section">
              <div className="section-header">
                <h2>怨꾩빟</h2>
                <button onClick={() => setShowContractModal(true)}>+ 怨꾩빟</button>
              </div>
              ...
              {showContractModal && (
                <form onSubmit={handleCreateContract} className="add-form">
                  ...
                  <div className="form-group"><input type="number" placeholder="湲덉븸" value={contractData.total_amount} onChange={(e) => setContractData({ ...contractData, total_amount: parseInt(e.target.value, 10) || 0 })} /></div>
                  <div className="form-actions">
                    <button type="submit">???/button>
                    <button type="button" onClick={() => setShowContractModal(false)}>痍⑥냼</button>
                  </div>
                </form>
              )}
              ...
              <div className="contracts-list">
                {contracts.map((contract) => (
                  <div key={contract.id} className={`contract-card ${contract.status}`}>
                    ...
                    <div className={`contract-status-badge ${contract.status}`}>{contractStatusLabels[contract.status] || contract.status}</div>
                    <div className="contract-amount">{Number(contract.total_amount || 0).toLocaleString()}??/div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {hardwareStatus.map((hw) => (
                  <div key={hw.id} className={`hardware-card ${hw.door_status}`}>
                    ...
                    <div className="hardware-status">
                      ...
                      {doorStatusLabels[hw.door_status] || hw.door_status}
                    </div>
                    {hw.fire_alarm && <div className="fire-alarm">?붿옱 寃쎈낫</div>}
                    <button className="unlock-btn" onClick={() => handleUnlockDoor(hw.warehouse_id)}>臾??닿린</button>
                    <div className="hardware-time">留덉?留??뺤씤: {new Date(hw.last_check).toLocaleString('ko-KR')}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'naver' && isAdmin && (
            <div className="naver-sync-section">
              <div className="section-header">
                <h2>?ㅼ씠踰??덉빟 ?숆린??/h2>
                <button onClick={() => setShowNaverSync(!showNaverSync)}>{showNaverSync ? '?リ린' : '?숆린??}</button>
              </div>
              {showNaverSync && (
                <div className="sync-actions">
                  <button onClick={handleSyncEmails} disabled={syncLoading}>{syncLoading ? '泥섎━ 以?..' : '?대찓???뚯떛'}</button>
                  <button onClick={handleSyncCrawler} disabled={syncLoading}>{syncLoading ? '泥섎━ 以?..' : '?щ·???ㅽ뻾'}</button>
                </div>
              )}
              <div className="reservation-list">
                {navReservations.length === 0 ? (
                  <p className="no-data">?덉빟 ?곗씠?곌? ?놁뒿?덈떎.</p>
                ) : (
                  navReservations.map((r) => (
                    <div key={r.id} className="reservation-card">
                      ...
                      <div className="reservation-status">{reservationStatusLabels[r.status] || r.status}</div>
                    </div>
                  ))
                )}
              </div>
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage('濡쒓렇?몃릺?덉뒿?덈떎. ??쒕낫?쒕줈 ?대룞?⑸땲??');
      setTimeout(() => navigate('/dashboard'), 1000);
    } catch (error) {
      setMessage(error.response?.data?.message || '?쒕쾭???곌껐?????놁뒿?덈떎.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label htmlFor="username">?꾩씠??/label>
            <input
              type="text"
              id="username"
              value={username}
        ...
        <p className="auth-link">
          怨꾩젙???꾩슂?섏떊媛?? <Link to="/register">?뚯썝媛??/Link>
        </p>
      </div>
    </div>
  );
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(`${response.data.message}. 濡쒓렇???붾㈃?쇰줈 ?대룞?⑸땲??`);
      setUsername('');
      setEmail('');
      setPassword('');
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        <p className="auth-link">
          ?대? 怨꾩젙???덉쑝?좉??? <Link to="/login">濡쒓렇??/Link>
        </p>
      </div>
    </div>
  );
...
const Profile = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setMessage('?꾨줈?꾩씠 ?섏젙?섏뿀?듬땲??');
      setEditMode(false);
      const updatedUser = { ...user, ...formData };
      localStorage.setItem('user', JSON.stringify(updatedUser));
      fetchProfile(user.id);
    } catch (error) {
      setMessage(error.response?.data?.message || '?섏젙???ㅽ뙣?덉뒿?덈떎.');
    }
  };
  ...
  if (!user) {
    return <div className="profile-container"><p>遺덈윭?ㅻ뒗 以?..</p></div>;
  }
    ...
    <div className="profile-container">
      <div className="profile-card">
        <div className="profile-header">
          <h2>?꾨줈??/h2>
          <button className="back-btn" onClick={() => navigate('/dashboard')}>?ㅻ줈</button>
        </div>
        ...
        {editMode ? (
          <form onSubmit={handleUpdate} className="profile-form">
            ...
            <div className="form-actions">
              <button type="submit">???/button>
              <button type="button" onClick={() => setEditMode(false)}>痍⑥냼</button>
            </div>
          </form>
        ) : (
          <div className="profile-info">
            <div className="info-item">
              <span className="info-label">?꾩씠??/span>
              <span className="info-value">{user.username}</span>
            </div>
            <div className="info-item">
              <span className="info-label">?대찓??/span>
              <span className="info-value">{user.email}</span>
            </div>
            <div className="info-item">
              <span className="info-label">?꾪솕踰덊샇</span>
              <span className="info-value">{user.phone || '-'}</span>
            </div>
            <div className="info-item">
              <span className="info-label">媛?낆씪</span>
              <span className="info-value">{new Date(user.created_at).toLocaleDateString('ko-KR')}</span>
            </div>
            <button className="edit-profile-btn" onClick={() => setEditMode(true)}>?꾨줈???섏젙</button>
          </div>
        )}

        {message && <p className="message">{message}</p>}

        <button className="logout-btn-full" onClick={handleLogout}>濡쒓렇?꾩썐</button>
      </div>
    </div>
  );
};
...
function LayoutEditor({ warehouseId: propWarehouseId, onBack }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('?덉씠?꾩썐??遺덈윭?ㅼ? 紐삵뻽?듬땲??', err);
        alert('?덉씠?꾩썐 ?곗씠?곕? 遺덈윭?ㅼ? 紐삵뻽?듬땲??');
        goBack();
      }
    };
    load();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert('?덉씠?꾩썐????λ릺?덉뒿?덈떎.');
    } catch (err) {
      console.error('?덉씠?꾩썐 ??μ뿉 ?ㅽ뙣?덉뒿?덈떎:', err);
      alert(`?덉씠?꾩썐 ??μ뿉 ?ㅽ뙣?덉뒿?덈떎: ${err.response?.data?.message || err.message}`);
    } finally {
      setSaving(false);
    }
  };

  if (loading) return <div className="loading">遺덈윭?ㅻ뒗 以?..</div>;
  if (!layout) return <div className="loading">李쎄퀬 ?곗씠?곌? ?놁뒿?덈떎.</div>;
  ...
  return (
    <div className="layout-editor">
      <div className="editor-header">
        <button onClick={goBack} className="btn-back">?ㅻ줈</button>
        <h2>{layout.name} - ?덉씠?꾩썐 ?몄쭛</h2>
        ...
        <div className="prop-panel">
          ...
          {dragging ? (
            <div className="prop-item">
              <p><strong>?대쫫:</strong> {dragging.name}</p>
              <p><strong>?ш린:</strong> {dragging.size}</p>
              <p><strong>?곹깭:</strong> {statusLabels[dragging.status] || dragging.status}</p>
              <p><strong>由대젅??</strong> {dragging.relay_channel}</p>
            </div>
          ) : (
            <p>罹먮퉬?룹쓣 ?쒕옒洹명븯??洹몃━??移몄뿉 ?볦쑝?몄슂.</p>
          )}
          <div className="legend">
            <h4>踰붾?</h4>
            <div className="legend-item"><span className="dot green"></span> ?ъ슜 以?/div>
            <div className="legend-item"><span className="dot blue"></span> ?ъ슜 媛??/div>
            <div className="legend-item"><span className="dot orange"></span> ?먭? 以?/div>
            <div className="legend-item"><span className="dot red"></span> 留뚮즺 ?꾨컯</div>
          </div>
        </div>
          ...
          <div className="grid-area" style={{ width: gridW, height: gridH }}>
            ...
            {cabinets.map((cab) => (
              ...
              >
                ...
                <div className={`cabinet-status status-${cab.status}`}>{statusLabels[cab.status] || cab.status}</div>
              </div>
            ))}
          </div>
        </div>
...
if (isProduction) {
  ...
  if (invalidSecrets.length > 0) {
    throw new Error(`운영 환경에서는 기본값이 아닌 시크릿이 필요합니다: ${invalidSecrets.join(', ')}`);
  }
}
...
app.use(cors({
  origin(origin, callback) {
    ...
    return callback(new Error('허용되지 않은 CORS 출처입니다'));
  },
  credentials: true,
}));
...
async function notifyUser(userId, title, body) {
  return new Promise(async (resolve) => {
    db.get(`SELECT phone FROM users WHERE id = ?`, [userId], (err, user) => {
      if (err || !user) { resolve({ success: false, error: '사용자를 찾을 수 없음' }); return; }
...
async function executeAutoBilling(contractId) {
  ...
  if (!billingKey) {
    ...
    return { success: false, error: '빌링 키 없음' };
  }
...
app.post('/api/register', async (req, res) => {
  ...
  console.log('[회원가입] 요청 본문:', JSON.stringify(req.body));
    ...
    db.get(`SELECT COUNT(*) AS cnt FROM users`, [], (err, row) => {
      ...
      db.run(`INSERT INTO users (username, email, password, phone, role, pin_code) VALUES (?, ?, ?, ?, ?, ?)`,
        [username, email, hashedPassword, phone || '', role, hashedPin], function (err) {
          ...
          console.log('[회원가입] 완료 userId:', this.lastID, '역할:', role);
          res.status(201).json({ message: '회원가입 완료', userId: this.lastID, role: role });
        });
    });
  } catch (error) {
    console.error('[회원가입] 예외:', error.message);
...
app.post('/api/warehouses', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.run(`INSERT INTO warehouses (name, location, capacity, owner_id) VALUES (?, ?, ?, ?)`,
    [name, location || '', capacity || 0, req.user.id], function (err) {
      ...
      res.status(201).json({ message: '창고가 생성되었습니다.', warehouseId: this.lastID });
    });
});
...
app.post('/api/warehouses/:warehouseId/cabinets', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) return res.status(500).json({ message: '서버 오류' });
      res.status(201).json({ message: '캐비넷이 추가되었습니다.', cabinetId: this.lastID });
    });
});
    ...
    db.get(`SELECT id FROM contracts WHERE cabinet_id = ? AND status = 'active' LIMIT 1`, [cabinet_id], (err, activeContract) => {
      ...
      db.run(`INSERT INTO contracts (user_id, cabinet_id, start_date, end_date, total_amount) VALUES (?, ?, ?, ?, ?)`,
        [contractUserId, cabinet_id, start_date, end_date, amount], function (err) {
          ...
          res.status(201).json({ message: '계약이 생성되었습니다.', contractId: this.lastID });
        });
    });
  });
});
...
app.post('/api/access/authenticate', rateLimitAccessAuth, checkAccessLockout, (req, res) => {
  ...
  function logAccess(uid, wid, method, success, note) {
    const source = getRequestSource(req);
    const auditNote = `${note} | 인증방식=${method} 창고=${wid} IP=${source.ip} 장치=${source.device} 사용자에이전트=${source.userAgent}`;
  ...
  function controlDoor(wid, action) {
    ...
    console.log(`[도어 제어] 창고 ${wid} - ${relayActionLabels[action] || action}`);
...
app.post('/api/admin/relay/control', authenticateToken, requireAdmin, requireHardwareSecret, async (req, res) => {
  ...
  try {
    ...
    logHardwareEvent(req, {
      warehouseId: warehouse_id,
      eventType: '릴레이_제어',
      note: `채널=${channel} 동작=${relayActionLabels[action] || action}`,
    });
    res.json({ message: `릴레이 ${relayActionLabels[action] || action} 완료` });
  } catch (err) {
    logHardwareEvent(req, {
      warehouseId: warehouse_id,
      eventType: '릴레이_제어',
      success: false,
      note: `채널=${channel} 동작=${relayActionLabels[action] || action} 오류=${err.message}`,
    });
...
function initSerialPort() {
  return new Promise((resolve) => {
    if (!HARDWARE_CONFIG.serialPort) {
      console.log('[?섎뱶?⑥뼱] SERIAL_PORT 誘몄꽕??- ?쒕??덉씠??紐⑤뱶');
      resolve();
      return;
    }
    ...
    try {
      ...
      port.on('open', () => {
        console.log(`[?섎뱶?⑥뼱] ?쒕━???ы듃 ?곌껐: ${HARDWARE_CONFIG.serialPort}`);
        resolve();
      });

      port.on('error', (err) => {
        console.error('[?섎뱶?⑥뼱] ?쒕━???ы듃 ?ㅻ쪟:', err.message);
        port = null;
        resolve();
      });
      ...
      console.log('[?섎뱶?⑥뼱] ?쒕━???ы듃 ?곌껐 ?ㅽ뙣 - ?쒕??덉씠??紐⑤뱶');
      port = null;
      resolve();
    }
  });
...
function controlRelay(warehouseId, channel, action) {
  return new Promise((resolve, reject) => {
    db.get(`SELECT relay_channel FROM cabinets WHERE id = ?`, [channel], (err, cabinet) => {
      ...
      console.log(`[由대젅?? 梨꾨꼸 ${relayChannel} ${action === 'open' ? '?닿린' : '?リ린'} (${hexCommand})`);
...
function unlockDoor(warehouseId, duration = HARDWARE_CONFIG.relayDelay) {
  ...
  db.get(`SELECT id FROM cabinets WHERE warehouse_id = ? LIMIT 1`, [warehouseId], (err, cabinet) => {
    if (err) {
      console.error('[?꾩뼱] 罹먮퉬??議고쉶 ?ㅻ쪟:', err.message);
      return;
    }
...
function handleFireAlarm(warehouseId) {
  console.log(`[?붿옱 寃쎈낫] 李쎄퀬 ${warehouseId} 媛뺤젣 媛쒕갑`);
  ...
  triggerAdminAlert(`?붿옱 寃쎈낫: 李쎄퀬 ${warehouseId} 紐⑤뱺 臾?媛뺤젣 媛쒕갑`);
}
      ...
      (err, rows) => {
        ...
        rows.forEach((row) => {
          ...
          if (diff > HARDWARE_CONFIG.doorTimeout) {
            triggerAdminAlert(`臾??대┝ 吏?? ${row.name} (${Math.floor(diff / 1000)}珥?`);
          }
        });
      },
    );
  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function onInsert(err) {
        ...
        if (this.changes > 0) {
          console.log(`[?ㅼ씠踰??숆린?? ?덉빟 ??λ맖: ${reservation.reservation_id}`);
        }
  ...
  return new Promise((resolve) => {
    msg.on('body', async (stream) => {
      try {
        ...
        console.error('[?ㅼ씠踰??숆린?? ?뚯떛 ?ㅻ쪟:', err.message);
        resolve(0);
      }
    });
  });
...
async function fetchEmails() {
  ...
  return new Promise((resolve, reject) => {
    if (!IMAP_CONFIG.user || !IMAP_CONFIG.password) {
      console.log('[?ㅼ씠踰??숆린?? EMAIL_USER ?먮뒗 EMAIL_PASSWORD媛 ?ㅼ젙?섏? ?딆븘 ?대찓???숆린?붾? 嫄대꼫?곷땲??);
      resolve(0);
      return;
    }
    ...
    imap.once('error', (err) => {
      console.error('[?ㅼ씠踰??숆린?? IMAP ?곌껐 ?ㅻ쪟:', err.message);
      finish(err);
    });
      ...
      imap.openBox('INBOX', false, (err) => {
        ...
        imap.search([['UNSEEN']], (searchErr, results) => {
          ...
          fetch.once('error', (fetchErr) => {
            console.error('[?ㅼ씠踰??숆린?? 媛?몄삤湲??ㅻ쪟:', fetchErr.message);
            finish(fetchErr);
          });
...
function auditCrawler(eventType, success, attempt, note = '') {
  ...
  console.log(`[?ㅼ씠踰??щ·?? ${eventType} ?쒕룄=${attempt || '-'} ?깃났=${success ? '?? : '?꾨땲??} ${note}`);
}
...
async function crawlNaverPartnerOnce() {
  ...
  try {
    ...
    if (loginFailed) throw new Error('?뚰듃??濡쒓렇???ㅽ뙣 ?먮뒗 怨꾩젙 ?좉툑 寃쎄퀬媛 媛먯??섏뿀?듬땲??);
...
async function crawlNaverPartner() {
  if (!CRAWLER_CONFIG.partnerId || !CRAWLER_CONFIG.partnerPw) {
    console.log('[?ㅼ씠踰??щ·?? NAVER_PARTNER_ID ?먮뒗 NAVER_PARTNER_PW媛 ?ㅼ젙?섏? ?딆븘 ?щ·???숆린?붾? 嫄대꼫?곷땲??);
    auditCrawler('?몄쬆?뺣낫_?꾨씫?쇰줈_嫄대꼫?', true, 0);
    return 0;
  }

  for (let attempt = 1; attempt <= CRAWLER_CONFIG.maxAttempts; attempt += 1) {
    try {
      auditCrawler('?쒕룄_?쒖옉', true, attempt);
      const count = await crawlNaverPartnerOnce();
      auditCrawler('?쒕룄_?깃났', true, attempt, `?숆린??${count}`);
      return count;
    } catch (err) {
      auditCrawler('?쒕룄_?ㅽ뙣', false, attempt, err.message);
      if (attempt === CRAWLER_CONFIG.maxAttempts) {
        console.error('[?ㅼ씠踰??щ·?? 理쒕? ?쒕룄 ?잛닔???꾨떖?섏뿬 怨꾩젙 ?좉툑???쇳븯湲??꾪빐 以묒??⑸땲??);
        return 0;
      }
...
function startSyncScheduler(intervalMs = 600000) {
  console.log(`[?ㅼ씠踰??숆린?? ?ㅼ?以꾨윭 ?쒖옉: ?대찓??${intervalMs}ms ?щ·??3600000ms`);

  const emailTimer = setInterval(async () => {
    try {
      const count = await fetchEmails();
      console.log(`[?ㅼ씠踰??숆린?? ?대찓??泥섎━?? ${count}`);
    } catch (err) {
      console.error('[?ㅼ씠踰??숆린?? ?대찓???ㅻ쪟:', err.message);
    }
  }, intervalMs);
  ...
  const crawlerTimer = setInterval(async () => {
    try {
      ...
      console.log(`[?ㅼ씠踰??щ·?? ?숆린?붾맖: ${count}`);
    } catch (err) {
      console.error('[?ㅼ씠踰??щ·?? ?ㅻ쪟:', err.message);
    }
  }, 3600000);
