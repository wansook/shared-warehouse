...
async function test() {
  ...
  console.log('[5] Create Contract:', contract.data.message, 'ID:', contract.data.contractId, '✅');

  const mockPayment = await request('POST', '/api/payments/mock', {
    contract_id: contract.data.contractId,
    amount: 150000
  }, auth);
  console.log('[6] Mock Payment:', mockPayment.data.message, mockPayment.data.paymentKey ? '✅' : '❌');
